...
Lexer::Lexer(const std::string& kaynakKodUtf8) : kaynakKod_(utf8ToU32(kaynakKodUtf8)) {
...
std::vector<Token> Lexer::tokenize() {
    ...
    while (!dosyaSonu()) {
        if (satirBasi) {
            ...
            if (c != U'\n' && c != U'\r' && c != U'#') {
                ...
                if (girintiSayisi > mevcutGirinti) {
                    ...
                    while (girintiYigini.size() > 1 && girintiSayisi < girintiYigini.back()) {
                        girintiYigini.pop_back();
                        tokenlar.push_back({TokenType::CIKINTI, "<CIKINTI>", satir_});
                    }
                    if (girintiSayisi != girintiYigini.back()) {
                        tokenlar.push_back({TokenType::HATA, "Geçersiz girinti seviyesi", satir_});
        ...
        const std::u32string tekKarakter(1, ilerle());
        tokenlar.push_back({TokenType::HATA, "Tanımsız karakter: " + u32ToUtf8(tekKarakter), satir_});
    }
...
std::u32string Lexer::utf8ToU32(const std::string& metin) {
    ...
    while (i < metin.size()) {
        ...
        if ((b0 & 0x80) == 0x00) {
            ...
            kodNoktasi = (static_cast<char32_t>(b0 & 0x1F) << 6) | static_cast<char32_t>(b1 & 0x3F);
            uzunluk = 2;
...
bool Lexer::dosyaSonu() const {
    return konum_ >= kaynakKod_.size();
}
...
bool Lexer::rakamMi(char32_t c) const {
    return c >= U'0' && c <= U'9';
}
