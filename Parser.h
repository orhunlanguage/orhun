...
private:
    ...
    std::unique_ptr<BlockNode> parseBlokVeyaTekKomut(const std::string& baglam, bool girintiZorunlu);
    ...
    const Token& tuket(TokenType tur, const std::string& deger, const std::string& hataMesaji);

    void yeniSatirlariAtla();
    bool ifadeBaslangiciMi(const Token& token) const;

    [[noreturn]] void syntaxError(const Token& token, const std::string& mesaj) const;
};
