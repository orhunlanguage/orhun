...
bool yerTutucuYoluGecerliMi(const std::string& yol) {
    ...
    while (true) {
        ...
        const std::string parca =
            nokta == std::string::npos ? yol.substr(bas)
...
std::wstring utf8denWstringe(const std::string& metin) {
    ...
    const int gerekli = MultiByteToWideChar(CP_UTF8, 0, metin.c_str(),
...
std::string internetIcerigiGetir(const std::string& url) {
    ...
    std::wstring yol = parcalar.dwUrlPathLength > 0
                           ? std::wstring(parcalar.lpszUrlPath,
                                          parcalar.dwUrlPathLength)
                           : L"/";
    ...
    HINTERNET istek = api.openRequest(baglanti, L"GET", yol.c_str(), nullptr,
                                      WINHTTP_NO_REFERER,
    ...
    if (api.queryHeaders(istek,
                         ...
                         WINHTTP_HEADER_NAME_BY_INDEX, &durumKodu,
                         &durumUzunluk, WINHTTP_NO_HEADER_INDEX)) {
        if (durumKodu >= 400) {
            api.closeHandle(istek);
            api.closeHandle(baglanti);
            api.closeHandle(oturum);
            throw std::runtime_error("HTTP durum kodu: " +
                                     std::to_string(durumKodu));
        }
    }
    ...
    while (true) {
        ...
        if (!api.queryDataAvailable(istek, &mevcut)) {
            ...
            api.closeHandle(baglanti);
            api.closeHandle(oturum);
            throw std::runtime_error("WinHttpQueryDataAvailable başarısız: " +
                                     hata);
        }
...
void Interpreter::gomuluIslevleriYukle() {
    ...
    gomuluIslevler_["json.coz"] = [this](const std::vector<OrhunDegeri>& args, std::size_t satir) -> OrhunDegeri {
        if (args.size() != 1) {
    ...
    gomuluIslevler_["ffi.cagir"] = [this](const std::vector<OrhunDegeri>& args,
        ...
        for (std::size_t i = 2; i < args.size(); ++i) {
            ...
            hataFirlat(satir, "ffi.cagir sadece int/double(tam sayı)/metin argümanlarını destekliyor.");
        }
    ...
    gomuluIslevler_["ffi.sembol_var_mi"] = [this](const std::vector<OrhunDegeri>& args,
        ...
        if (!std::holds_alternative<std::string>(args[1].veri)) {
            hataFirlat(satir, "ffi.sembol_var_mi içinde fonksiyon adı metin olmalıdır.");
        }
    ...
    gomuluIslevler_["ffi.cagir_metin"] = [this](const std::vector<OrhunDegeri>& args,
        ...
        for (std::size_t i = 2; i < args.size(); ++i) {
            ...
            if (std::holds_alternative<double>(arg.veri)) {
                const double d = std::get<double>(arg.veri);
                if (!tamSayiMi(d)) {
                    hataFirlat(satir, "ffi.cagir_metin şimdilik ondalık argümanları desteklemiyor (arg #" +
                                          std::to_string(i - 1) + ").");
                }
                hamArgumanlar.push_back(static_cast<std::intptr_t>(d));
                continue;
            ...
            hataFirlat(satir, "ffi.cagir_metin sadece int/double(tam sayı)/metin argümanlarını destekliyor.");
        }
...
OrhunDegeri Interpreter::dilimErisim(const DilimErisimNode* dugum) {
    ...
    if (std::holds_alternative<OrhunDegeri::ListeTipi>(hedef.veri)) {
        ...
        if (!listePtr) {
            hataFirlat(dugum->satir(), "Dilim erişiminde geçersiz boş liste referansı.");
        }

        const auto& liste = *listePtr;
        const long long uzunluk = static_cast<long long>(liste.size());
        long long baslangic = dilimSiniriCevir(dugum->baslangic(), 0, uzunluk,
                                               dugum->satir(),
    ...
    if (std::holds_alternative<std::string>(hedef.veri)) {
        ...
        return OrhunDegeri(metin.substr(static_cast<std::size_t>(baslangic),
...
std::string Interpreter::metinGomuleriCoz(const std::string& metin,
    ...
    while (i < metin.size()) {
        ...
        if (c == '{') {
            ...
            const std::string hamYerTutucu =
                metin.substr(i + 1, kapanis - (i + 1));
